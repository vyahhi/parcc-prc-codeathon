...
(function ($) {
  ...
  retryAjax = function($element, settings, parent_value, default_value, base_id){
    failures ++;
    if(failures < 3){
      this.getTermChildren($element, settings, parent_value, default_value, base_id);
    }else{
      console.log('Failed to retrieve data from the server after 3 attempts.')
    }
  }
    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success == true) {
          ...
          retryAjax($element, settings, parent_value, default_value, base_id);
        }
      },
